...
public class MathUtils {
     ...
     * @return
     */
    public static int fibonacciWithFor(int number) {
        int preValue = 0;
        int prePreValue = 1;
        int current = prePreValue + prePreValue;

        for(int i = 0; i < number; i++) {
            current = prePreValue + prePreValue;
    ...
    public static int findNumber(int[] array, int i) {
        return findNumber(array, i, 0, array.length, -1);
    }

    private static int findNumber(int[] array, int i, int min, int max, int lastIndex) {
        ...
        if(array[middle] < i) {
            return findNumber(array, i, middle, max, middle);
        } else if(array[middle] > i) {
            return findNumber(array, i, min, middle, middle);
        } else {
            return middle;
        }
    }
